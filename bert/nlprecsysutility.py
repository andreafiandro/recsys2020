...
class RecSysUtility:
    ...
    def create_chunk_csv(self, output_dir='./test_chunk', chunk_size = 1000000):
        ...
        for chunk in pd.read_csv(self.test_file, sep='\u0001', header=None, chunksize=chunk_size):
            df_chunk = self.process_chunk_tsv(chunk,isVal=False)
...
# submission utility function

def submission_files(submission_dataframe, chunkname, output_dir="./"):
        submission_dataframe.to_csv(os.path.join(output_dir,chunkname+'_reply_prediction.csv'),columns = ['Tweet_Id','User_Id_Engaging','Reply'],index=False, header = False)
        submission_dataframe.to_csv(os.path.join(output_dir,chunkname+'_retweet_prediction.csv'),columns = ['Tweet_Id','User_Id_Engaging','Retweet'],index=False, header = False)
        submission_dataframe.to_csv(os.path.join(output_dir,chunkname+'_retweet_with_comment_prediction.csv'),columns = ['Tweet_Id','User_Id_Engaging','Retweet_with_comment'],index=False , header = False)
        submission_dataframe.to_csv(os.path.join(output_dir,chunkname+'_like_prediction.csv'),columns = ['Tweet_Id','User_Id_Engaging','Like'],index=False, header = False)
