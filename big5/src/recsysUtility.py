...
class RecSysUtility:
    ...
    def create_chunk_csv(self, output_dir='../input/val', chunk_size = 1000000):
        ...
        for chunk in pd.read_csv(self.training_file, sep='\u0001', header=None, chunksize=chunk_size):
            df_chunk = self.process_chunk_tsv(chunk, isVal=True)
            df_chunk.to_csv(os.path.join(output_dir, 'val_chunk_{}.csv'.format(chunk_n)), index=False)
            chunk_n += 1
